class ApplicationController < ActionController::Base
  ...
  def find_by_id_or_redirect(model, id = params[:id])
    ...
    if instance.nil?
      respond_to do |format|
        format.html {
          path = model == Version ? changes_path : polymorphic_path(params[:controller].split(/\//)[0..-1])
  ...
  private
	...
	def set_locale
		I18n.locale = params[:locale] || session[:locale] || request.env['HTTP_ACCEPT_LANGUAGE'].scan(/^[a-z]{2}/).first rescue I18n.default_locale
