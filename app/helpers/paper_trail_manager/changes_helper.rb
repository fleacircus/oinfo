class PaperTrailManager
  module ChangesHelper
    ...
    # Returns HTML link for the item stored in the version
    ...
    # Returns HTML link for whodunnit stored in the version
    def change_whodunnit_link(version)
      if PaperTrailManager.whodunnit_class && version.whodunnit
        if user = PaperTrailManager.whodunnit_class.find(version.whodunnit) rescue nil
            return link_to(h(user.send(PaperTrailManager.whodunnit_name_method)), user_path(user))
        end
      end
    end
